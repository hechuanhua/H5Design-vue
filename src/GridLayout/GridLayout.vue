...
console.log(elementResizeDetectorMaker, 'elementResizeDetectorMaker');
export default {
	name: 'GridLayout',
	provide() {
		return {
			...
			this.$nextTick(function () {
				...
				self.$nextTick(function () {
					...
					this.erd.listenTo(self.$refs.item, function () {
						console.log('elementResizeDetectorMaker => listenTo');
						self.onWindowResize();
					});
				});
			});
	...
	methods: {
		...
		containerHeight: function () {
			...
			const containerHeight =
				bottom(this.layout) * (this.rowHeight + this.margin[1]) + this.margin[1] + 'px';
