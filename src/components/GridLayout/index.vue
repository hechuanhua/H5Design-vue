<template>
	...
	>
		...
		>
			<span
				class="formKey"
				v-if="type === PageType.EDIT && item.type === ComponentsType.COMMONCONTAINER"
			>
				{{ item.config.formKey }}
			</span>
			<span class="remove" @click="removeItem(item)" v-if="type === PageType.EDIT">x</span>
			<Render
				:layoutItem="item"
				:dragData="dragData"
				:type="type"
...
import { ComponentsInfo, ComponentsType, LayoutDataItem, PageType } from '@/typings/Common';
...
const drop = (event: DragEvent) => {
	...
	if (tempData) {
		...
		const position = {
			x: Math.round((event.pageX - ret.left) / (gridLayoutConfig.width / gridLayoutConfig.colNum)),
