<template>
	<div :class="`components-${propsLayoutItem.type} h100`">
		...
		>
			<div class="filter tr" v-if="echartData && propsLayoutItem.config.is_filter">
				筛选：
				<Select
		...
		<Button v-if="propsLayoutItem.type === ComponentsType.BUTTON" :type="'primary'">
		...
		<Tabs v-if="propsLayoutItem.type === ComponentsType.TABS" @change="onChange" :type="'card'">
			<TabPane
...
import { Input, Select, RangePicker, Table, Button, Empty, Tabs, Radio } from 'ant-design-vue';
...
import { useEchartsBarData, useEchartsTableData, useEchartsPieData } from './useData';
...
const echartFilterChange = (value: any) => {
	...
	const series = cloneEchartData.series.filter((item: any) => value.includes(item.name));
	if (series.length) {
...
const getFormDataParams = () => {
	...
	const mergeParams = JSON.parse(
		JSON.stringify(Object.assign({}, previewStore.params['common'], currentContainerParams))
	);
...
const fetchPageData = () => {
	...
	if (!propsLayoutItem.value.config.api || !propsLayoutItem.value.config.api.url) return;
